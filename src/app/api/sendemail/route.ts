...
export async function DELETE(req: Request) {
  try {
    ...
    console.error('Error deleting product:', error); // Using error variable
    if (error instanceof Error) {
      return new Response(JSON.stringify({ error: 'Failed to delete product', details: error.message }), { status: 500, headers: { 'Content-Type': 'application/json' } });
    }
    return new Response(JSON.stringify({ error: 'Failed to delete product', details: 'An unknown error occurred' }), { status: 500, headers: { 'Content-Type': 'application/json' } });
  }
}